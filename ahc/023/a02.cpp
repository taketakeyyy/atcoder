...
public:
    ...
    void rest_crops(vector<Crop> rest_crops) {m_rest_crops = rest_crops;}
    vector<Crop> rest_crops() const {return m_rest_crops;}
...
public:
    ...
    void plan_value_greedy(ll now_turn, State &state, ll upper_value, ll upper_use_size, bool history_flag) {
        ...
        // 価値が上限値を超えるcropは除外する
        for(auto crop: state.rest_crops()) {
            if (crop.value() > upper_value) continue;
            value_order_crops.push_back(crop);
        }
    ...
    // 今のターンに植えられるものを価値の高い順に貪欲に植える
    void plan_s_greedy(ll now_turn, State &state, bool history_flag) {
        // もう植えられないものを除外する
        state.exclude_expired_crops(now_turn);

        // 今のターンに植えられるものだけを残す
        vector<Crop> proc_crops;
        for(Crop crop: state.rest_crops()) {
            if (crop.s() == now_turn) proc_crops.push_back(crop);
        }

        ll use_size = min((ll)proc_crops.size(), state.available_section());

        // 植える処理をします
        do_plant(proc_crops, state, use_size, now_turn, history_flag);
    ...
    State beam_search(ll now_turn, State now_state) {
        const ll BEAM_WIDTH = 2; // ビーム幅
        const ll BEAM_DEPTH = 2; // ビーム深さ
...
void solve() {
    ...
    solver.run(init_state);
    solver.best_output();

    // auto end = std::chrono::system_clock::now();
    // auto msec = std::chrono::duration_cast<std::chrono::milliseconds>(end-start).count();
    // // [TODO]提出時は消すこと 要した時間をミリ秒（1/1000秒）に変換して表示
