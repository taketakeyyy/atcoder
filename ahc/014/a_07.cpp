...
// const clock_t TIME_LIMIT_SOLVER = 120*CLOCKS_PER_SEC;
 ...
 * @param line_set
 */
vector<RECT> out_in_solver(time_t start_clock, ll max_len, vector<vector<ll>> &grid, set<Line> &line_set) {
    set<pair<ll,ll>> outer_nonpoint_set;  // 外側の印がない点群
    set<pair<ll,ll>> inner_nonpoint_set;  // 内側の印がない点群
    vector<pair<ll,ll>> outer_nonpoints;  // 外側の印がない点リスト
    vector<pair<ll,ll>> inner_nonpoints;  // 内側の印がない点リスト
    for(ll x=0; x<N; x++) {
        for(ll y=0; y<N; y++) {
            if (N/4 <= x && x <= 3*N/4 && N/4 <= y && y <= 3*N/4) {
                if (grid[x][y] == 0) {
                    inner_nonpoint_set.insert({x,y});
                    inner_nonpoints.push_back({x,y});
                }
            }
            else {
                if (grid[x][y] == 0) {
                    outer_nonpoint_set.insert({x,y});
                    outer_nonpoints.push_back({x,y});
                }
            }
        }
    }
    ...
    ll inner_max_len = max_len;
    ll outer_max_len = max_len;
    shuffle(outer_nonpoints.begin(), outer_nonpoints.end(), engine);
    shuffle(inner_nonpoints.begin(), inner_nonpoints.end(), engine);
    while(1) {
        if (clock() - start_clock > TIME_LIMIT_SOLVER) break;

        bool is_found = false;
        // 外側を優先して作る
        for(auto [sx, sy]: outer_nonpoints) {
            if (grid[sx][sy] != 0) continue;
            RECT res_rect;
            bool is_ok = make_rect_r(sx, sy, outer_max_len, grid, line_set, res_rect);
            if (!is_ok) continue;
            is_found = true;
            ans.push_back(res_rect);
        }
        ...
        // 内側を作る
        for(auto [sx, sy]: inner_nonpoints) {
            if (grid[sx][sy] != 0) continue;
            RECT res_rect;
            bool is_ok = make_rect_r(sx, sy, inner_max_len, grid, line_set, res_rect);
            if (!is_ok) continue;
            is_found = true;
            ans.push_back(res_rect);
        }
        ...
        if (!is_found && outer_max_len >= N && inner_max_len >= N) break;

        if (!is_found) {
            outer_max_len = min(outer_max_len*2, N);
            inner_max_len = min(inner_max_len*2, N);
        }
    }
...
struct State2 {
    ...
    ll inner_max_len;
    ll outer_max_len;
    // vector<pair<ll,ll>> inner_nonpoints;
    // vector<pair<ll,ll>> outer_nonpoints;
    vector<vector<ll>> grid;
    set<Line> line_set;
    vector<RECT> rects;
    ll last_pos;
...
bool outer_search(vector<pair<ll,ll>> &outer_nonpoints, vector<vector<ll>> &grid, ll &max_len, set<Line> &line_set, vector<RECT> &ans) {
    bool is_found = false;
    for(auto [sx, sy]: outer_nonpoints) {
        if (grid[sx][sy] != 0) continue;
        RECT res_rect;
        bool is_ok = make_rect_r(sx, sy, max_len, grid, line_set, res_rect);
        if (!is_ok) continue;
        is_found = true;
        ans.push_back(res_rect);
    }
...
bool inner_search(vector<pair<ll,ll>> &inner_nonpoints, vector<vector<ll>> &grid, ll &max_len, set<Line> &line_set, vector<RECT> &ans) {
    bool is_found = false;
    for(auto [sx, sy]: inner_nonpoints) {
        if (grid[sx][sy] != 0) continue;
        RECT res_rect;
 ...
 * @param line_set
 */
vector<RECT> out_in_beam_solver(time_t start_clock, ll max_len, vector<vector<ll>> &grid, set<Line> &line_set) {
    set<pair<ll,ll>> outer_nonpoint_set;  // 外側の印がない点群
    set<pair<ll,ll>> inner_nonpoint_set;  // 内側の印がない点群
    vector<pair<ll,ll>> outer_nonpoints;  // 外側の印がない点リスト
    vector<pair<ll,ll>> inner_nonpoints;  // 内側の印がない点リスト
    ...
    shuffle(inner_nonpoints.begin(), inner_nonpoints.end(), engine);
    ll BEAM_WIDTH = 5;
    vector<vector<State2>> beam;
    // ビーム初期状態
    {
        beam.push_back(vector<State2>(BEAM_WIDTH));
        ll init_score = evaluate_score(grid);
        for(ll bw=0; bw<BEAM_WIDTH; bw++) {
            // ll inner_max_len_ = min(max_len+bw*2, N);
            // ll outer_max_len_ = min(max_len+bw*2, N);
            ll inner_max_len_ = min(max_len+bw*1, N);
            ll outer_max_len_ = min(max_len+bw*1, N);
            beam[0][bw] = State2{init_score, inner_max_len_, outer_max_len_, grid, line_set, vector<RECT>(), -1, false};
        }
    }
    ...
    // ビームサーチ
    {
        ...
        while(1) {
            ...
            {
                if (clock() - start_clock > TIME_LIMIT_SOLVER) break;
                bool is_all_end = true;
                for(ll bw=0; bw<BEAM_WIDTH; bw++) {
                    is_all_end &= beam[bi][bw].end;
                }
                if (is_all_end) break;
            }
            ...
            // 今回の状態を候補にあらかじめ入れておく
            vector<State2> candidates;
            for(ll bw=0; bw<BEAM_WIDTH; bw++) {
                candidates.push_back(beam[bi][bw]);
            }
            ...
            for(ll bw=0; bw<BEAM_WIDTH; bw++) {
                ...
                // 状態を取り出す
                if (beam[bi][bw].end) continue;

                // 外側のみ
                {
                    State2 state = beam[bi][bw];
                    state.rects.resize(0);
                    bool is_found = false;
                    is_found |= outer_search(outer_nonpoints, state.grid, state.outer_max_len, state.line_set, state.rects);
                    if (!is_found && state.outer_max_len >= N && state.inner_max_len >= N) {
                        state.end = true;
                    }
                    if (!is_found) {
                        state.outer_max_len = min(state.outer_max_len*2, N);
                    }
                    state.score = state.score + evaluate_score(state.rects);
                    state.last_pos = bw;
                ...
                // 内側のみ
                {
                    ...
                    state.rects.resize(0);
                    bool is_found = false;
                    is_found |= inner_search(inner_nonpoints, state.grid, state.inner_max_len, state.line_set, state.rects);
                    if (!is_found && state.outer_max_len >= N && state.inner_max_len >= N) {
                        state.end = true;
                    }
                    if (!is_found) {
                        state.inner_max_len = min(state.inner_max_len*2, N);
                    }
                    state.score = state.score + evaluate_score(state.rects);
                    state.last_pos = bw;
                    candidates.push_back(state);
    ...
    {
        ll bw = 0;
        for(ll bi=beam.size()-1; bi>=0; bi--) {
            for(ll ri=(ll)beam[bi][bw].rects.size()-1; ri>=0; ri--) {
                ans.push_back(beam[bi][bw].rects[ri]);
            }
            bw = beam[bi][bw].last_pos;
            if (bw == -1) break;
        }
    }
    reverse(ans.begin(), ans.end());

    grid = beam[beam.size()-1][0].grid;
    return ans;
}
...
void solve() {
    ...
    while(clock() - start_clock <= TIME_LIMIT_SOLVER) {
        for(auto max_len: max_len_table) {
            ...
            //     vector new_grid = grid;
            //     set<Line> line_set;  // ((x1,y1) (x2,y2)) を結ぶ線が存在する
            //     vector history = out_in_beam_solver(start_clock, max_len, new_grid, line_set);
            //     ll score = evaluate_score(new_grid);
            //     if (score > best_score) {
            //         best_history = history;
            //         best_score = score;
            ...
            if (clock() - start_clock >= TIME_LIMIT_SOLVER) break;
            {
                vector new_grid = grid;
                set<Line> line_set;  // ((x1,y1) (x2,y2)) を結ぶ線が存在する
                vector history = out_in_solver(start_clock, max_len, new_grid, line_set);
                ll score = evaluate_score(new_grid);
                if (score > best_score) {
                    best_history = history;
                    best_score = score;
