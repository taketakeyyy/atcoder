...
vector<RECT> naive_search_solver(clock_t start_clock, ll max_len, set<Point> const &init_point_set, ll &res_score) {
    vector<RECT> ans;
    set<Line> line_set;
    set<Point> point_set = init_point_set;

    while(1) {
        set<Point> proc_point_set = point_set;  // ループ用
        bool is_found = false;

        for(auto point: proc_point_set) {
            ll sx = point.x;
            ll sy = point.y;
            auto rects_and_line_sets = make_rect_sequence(sx, sy, max_len, point_set, line_set);
        ...
        if (!is_found && max_len >= N) break;
    }

    res_score = evaluate_score(point_set);
