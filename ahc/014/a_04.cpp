...
vector<RECT> second_solver2(clock_t start_clock, ll max_len, vector<vector<ll>> &grid, set<Line> &line_set) {
    vector<RECT> ans;
    ...
    // shuffle(XYs.begin(), XYs.end(), engine);
    ...
    while(do_loop || max_len<N) {
        ...
        shuffle(XYs.begin(), XYs.end(), engine);
        vector loop_XYs = XYs;  // ループ用
        for(auto xy: loop_XYs) {
            auto [x, y] = xy;
            if (grid[x][y] == 0) continue;
    ...
    for(auto max_len: max_len_table) {
        while(clock() - start_clock <= TIME_LIMIT_SOLVER) {
...

            {
                vector new_grid = grid;
                set<Line> line_set;  // ((x1,y1) (x2,y2)) を結ぶ線が存在する
                vector history = second_solver2(start_clock, max_len, new_grid, line_set);
                ll score = evaluate_score(new_grid);
                if (score > best_score) {
                    best_history = history;
                    best_score = score;
