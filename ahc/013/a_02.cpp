...
class Solver {
    public:
         ...
         */
        void move_strategy() {
            clock_t clock_start = clock();  // クロックスタート
            clock_t MAX_CLOCK = 1.5 * CLOCKS_PER_SEC;

            while(!unconnected_ids.empty()) {
                if (clock() - clock_start > MAX_CLOCK) break;
                // のちのクラスタ結合の得点の上げ幅が大きいと思われるので、ある程度行動回数を残す
                ll MARGIN = (ll)(uf.get_group_num()/3);
        ...
        void maximum_cluster() {
            ...
            for(ll id: ids) {
                ...
                ll target_color = grid[si][sj];
                // 上に走査
                if ((ll)connect_list.size() + (ll)move_list.size() + 1LL > MAX_ACTION_NUM) break;
                for(ll i=si-1; i>=0; i--) {
