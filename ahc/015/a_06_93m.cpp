 ...
 * @return pair<ll,ll>
 */
pair<ll,ll> put_candy(ll p, vector<vector<ll>> const &grid) {
    ll cnt = 0;
    for(ll i=0; i<N; i++) {
        for(ll j=0; j<N; j++) {
            if (grid[i][j] == 0) {
...
void future_prediction(ll turn, ll depth, vector<vector<ll>> org_grid, double &best_score) {
    if ((turn == (N*N)-1) || (depth == 0)) {
        best_score = max(best_score, evaluate_score(org_grid));
        return;
    }

    // 適当に予想してキャンディーを置く
    std::uniform_int_distribution<> dist_p(1, 101-(turn+1));  // 一様分布関数（離散）[1, MAX]
    ll FEATURE_WIDTH = 2;
    for(ll k=0; k<FEATURE_WIDTH; k++) {  // 幅を持たせて予想する
        vector<vector<ll>> now_grid = org_grid;
        ll p = dist_p(engine); p--;
        auto[y, x] = put_candy(p, now_grid);
        now_grid[y][x] = f[turn];  // キャンディーを置く

        // 傾ける
        vector<vector<ll>> gridR = now_grid;
        vector<vector<ll>> gridL = now_grid;
        vector<vector<ll>> gridF = now_grid;
        vector<vector<ll>> gridB = now_grid;
        move_R(gridR);
        move_L(gridL);
        move_F(gridF);
        move_B(gridB);
        ...
        // 予想する
        future_prediction(turn+1, depth-1, gridR, best_score);
        future_prediction(turn+1, depth-1, gridL, best_score);
        future_prediction(turn+1, depth-1, gridF, best_score);
        future_prediction(turn+1, depth-1, gridB, best_score);
    }
}
...
void init_solver() {
    ...
    for(ll t=0; t<N*N; t++) {
        ll p; cin >> p; p--;
        auto[y, x] = put_candy(p, grid);
        grid[y][x] = f[t];  // キャンディーを置く

        // 傾ける
        vector<vector<ll>> gridR = grid;
        vector<vector<ll>> gridL = grid;
        vector<vector<ll>> gridF = grid;
        vector<vector<ll>> gridB = grid;
        move_R(gridR);
        move_L(gridL);
        move_F(gridF);
        move_B(gridB);

        // ある程度先読みして、最もスコアが上がりやすいやつを選択する
        double scoreR=0, scoreL=0, scoreF=0, scoreB=0;
        future_prediction(t+1, FUTURE_DEPTH, gridR, scoreR);
        future_prediction(t+1, FUTURE_DEPTH, gridL, scoreL);
        future_prediction(t+1, FUTURE_DEPTH, gridF, scoreF);
        future_prediction(t+1, FUTURE_DEPTH, gridB, scoreB);

        map<double,pair<char, vector<vector<ll>>>> dirmp;  // dirmp[スコア] := {'R', gridR}
        dirmp[scoreR] = {'R', gridR};
        dirmp[scoreL] = {'L', gridL};
        dirmp[scoreF] = {'F', gridF};
        dirmp[scoreB] = {'B', gridB};
        vector<double> scores = {scoreR, scoreL, scoreF, scoreB};
        sort(scores.begin(), scores.end());
        reverse(scores.begin(), scores.end());
        char dir;
        tie(dir, grid) = dirmp[scores[0]];
