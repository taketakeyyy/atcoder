...
void future_prediction(ll turn, ll depth, vector<vector<ll>> now_grid, double &best_score) {
    if ((turn == (N*N)-1) || (depth == 0)) {
        best_score = max(best_score, evaluate_score(now_grid));
        return;
    }

    // 適当に予想してキャンディーを置く
    std::uniform_int_distribution<> dist_p(1, 101-(turn+1));  // 一様分布関数（離散）[1, MAX]
    // std::uniform_int_distribution<> dist_p(1, 101-(turn));  // 一様分布関数（離散）[1, MAX]
...
void init_solver() {
    ...
    for(ll t=0; t<N*N; t++) {
        ...
        // ある程度先読みして、最もスコアが上がりやすいやつを多数決する
