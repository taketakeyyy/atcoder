 ...
 */
void init_solver() {
    std::uniform_int_distribution<> dist_move(1, 4);  // 一様分布関数（離散）[1, 4]
    ...
    for(ll t=0; t<N*N; t++) {
        ...
        move_F(gridF);
        // move_B(gridB);
        double scoreR = evaluate_score(gridR);
        double scoreL = evaluate_score(gridL);
        double scoreF = evaluate_score(gridF);
        // double scoreB = evaluate_score(gridB);
        ...
        vector<double> scores = {scoreR, scoreL, scoreF};
