...
void solve() {
    ...
    set<pair<ll,ll>> condition_set; // 同じグループになってはいけない条件の集合
    ll K; cin >> K;
    for(ll i=0; i<K; i++) {
        ll x, y; cin >> x >> y;
        x--; y--;
        ll rx = uf.find_root(x);
        ll ry = uf.find_root(y);
        condition_set.insert({min(rx,ry), max(rx,ry)});
    }
    ...
    for(ll query=0; query<Q; query++) {
        ll p, q; cin >> p >> q; p--; q--;
        ll rp = uf.find_root(p);
        ll rq = uf.find_root(q);

        if (condition_set.count({min(rp,rq), max(rp,rq)})) {
            cout << "No" << endl;
        }
        else {
            cout << "Yes" << endl;
