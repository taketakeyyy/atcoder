...
void solve() {
    ...
    auto dfs = [&](auto self, ll u, vector<bool> &visited, ll start, vector<ll> &history, vector<bool> &seen) -> ll {
        for(ll v: G[u]) {
            if (visited[v]) return v;
            visited[v] = true;
            seen[v] = true;
            history.push_back(v);
            ll res = self(self, v, visited, start, history, seen);
            if (res > -1) return res;
            visited[v] = false;
            history.pop_back();
        }
    ...
    for(ll i=0; i<N; i++) {
        ...
        vector<ll> history = {i};
        ll res = dfs(dfs, i, visited, i, history, seen);
        if (res > -1) {
            bool is_ok = false;
            vector<ll> ans_history;
            for(ll v: history) {
                if (v == res) is_ok = true;
                if (!is_ok) continue;
                ans_history.push_back(v);
            }

            cout << ans_history.size() << endl;
            for(ll v: ans_history) {
                cout << v+1 << " ";
            }
            cout << endl;
            return;
