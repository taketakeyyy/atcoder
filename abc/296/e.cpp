...
void solve() {
    ...
    // グラフ作成
    // サイズ1のサイクルと、サイズ1の非サイクルはSCCでは見分けがつかないので、事前に答えに足す
    vector<set<long long>> G(N); // グラフ
    ll ans = 0;
    for(ll i=0; i<N; i++) {
        ll a; cin >> a;
        a--;
        G[i].insert(a);
        if (i == a) ans++; // サイズ1のサイクルは事前に足す
    }

    // SCC実行
    SCC scc = SCC<long long>(N, G);
    auto groups = scc.scc_groups();

    // SCCの各グループにおいて、サイクルのサイズを足していく
