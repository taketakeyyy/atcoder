...
struct UnionFindVerSize {
    ...
    public:
        ...
        T get_edge_size(T x) {
            return this->edge_num[this->find_root(x)];
        }
...
void solve() {
    ...
    // 各連結成分の頂点数と辺数が同じか判定する
    bool is_ok = true;
    for(ll u=0; u<N; u++) {
        if (uf.get_size(u) == uf.get_edge_size(u)) continue;
        is_ok = false; break;
    }
    if (is_ok) {
        cout << "Yes" << endl;
