...
struct Clock {
    ...
    void shift(int diff_second) {
        ...
        if (second + diff_second >= 60) {
            diff_m += (second+diff_second)/60;
            second = (second+diff_second)%60;
        }
        else if (second + diff_second < 0) {
            diff_m --;
            second = 60+(second+diff_second);
        }
        else {
        ...
        if (minute + diff_m >= 60) {
            diff_h ++;
            minute = (minute+diff_m)%60;
        }
        else if(minute + diff_m < 0) {
            diff_h --;
            minute = 60+(minute+diff_m);
        }
        else {
            minute = minute+diff_m;
        }

        // cout << diff_h << " " << diff_m << " " << diff_second << endl;;
        // cout << hour << " " << minute << " " << second << endl;

        if (hour + diff_h >= 24) {
            hour = (hour+diff_h)%24;
        }
        else if (hour + diff_h < 0) {
            hour = (24+(hour+diff_h)%24)%24;
        }
        else {
            hour = hour + diff_h;
        }
