...
void solve() {
    ll W, N; cin >> W >> N;
    vector<ll> L, R, V;
    L.emplace_back(0);  // dammy
    R.emplace_back(0);  // dammy
    V.emplace_back(0);  // dammy
    for(int i=0; i<N; i++) {
        ll l, r, v; cin >> l >> r >> v;
        L.emplace_back(l);
        R.emplace_back(r);
    ...
    // 貰うDP + RMQ
    // dp[i][w] := 左からi番目まで見たときの、消費香辛料wのときの価値の最大値
    ...
    for(ll i=1; i<=N; i++) {
        ...
        for(ll w=0; w<=W; w++) {
            ...
            // 選ぶ場合
            ll l = max(0LL, w-R[i]);
            ll r = max(0LL, w-L[i]+1);
            if (r==0) continue;
            ll mx = -lazyrmq.query(l, r);
            if (mx == -1) continue;
            dp[i][w] = max(dp[i][w], mx + V[i]);  // 区間の中で最も大きい値に更新する
        }
    }
